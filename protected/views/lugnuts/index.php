	<div>
		<h1>Lug Nut & Backspacing Guide</h1>
...
<section class="all-makes cars_ul bdb_1">
			<h2 class="section-name_2">Choose Your Car</h2>
				...
				<?php $key=1;foreach (AutoMake::getAllFront() as $makeUrl=>$makeTitle):?>
					<li><a title="<?=$makeTitle?> wheels" href="/lugnuts<?=$makeUrl?>"><?=$makeTitle?></a></li>
					<?php if ($key%7 ==0):?>
					</ul><ul>
					<?php endif;?>
				<?php $key++;endforeach;?>
		...
		<section class="seo-text">
			<?=SiteConfig::getInstance()->getValue('lug_nuts_footer_text_block')?>
		</section>
	</div>
